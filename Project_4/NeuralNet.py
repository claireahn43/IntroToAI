...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        inActs = [1.0] + inActs
        #insert a bias of 1 to the inputs
        weightedSum = self.getWeightedSum(inActs)
        #compute weighted sum of the inputs
        return self.sigmoid(weightedSum) #compute sigmoid of the sum and return
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        weightedSum = self.getWeightedSum(inActs)
        #compute weighted sum of the inputs
        return self.sigmoidDeriv(weightedSum) #find the derivative of the weighted sum and return
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        """YOUR CODE"""
        inActs = [1.0] + inActs
        #prepend a bias of 1 to the inputs
        i = 0
        for weight in self.weights: #for all the weights in perceptron
            self.weights[i] = weight + alpha * delta * inActs[i] 
            #add modification and input for current weight
            totalModification += abs(alpha * delta * inActs[i])
            #add abs value of modification to total
            i += 1

        return totalModification #return sum of modifications
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        """
        """YOUR CODE"""
        result = [inActs] #putting input list as the first list
        i = 0
        for layer in self.layers: #for each layer
            currentLayer = []
            for node in layer: #for each perceptron in the current layer
            	value = node.sigmoidActivation(result[i]) 
            	#find value of the sigmoid of current perceptron's weights
            	currentLayer.append(value) 
            	#add the value to currentLayer list
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in range(len(example[1])):
                ...
                #find activation function derivative of the current perceptron
                error = example[1][outputNum] - lastLayerOutput[outputNum]
                #find error of the current perceptron yj - aj
                delta = gPrime * error
                #g'(inj)(yj-aj)
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    i = 0
                    j = 0
                    while i < len(nextLayer): 
                    	j = neuronNum + 1
                    	error += nextLayer[i].weights[j] * deltas[0][i]
                    	i += 1
                        #sum over all the differnt next layer neurons that is connected to current neuron
                        #calculate product of weight and their corresponding deltas 
                    delta = gPrime * error
                    #g'(ini) x sumj(wi,j delta[j])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    """
        ...
        #    print '.',
    weightMod += 0.1
    while (not weightMod <= weightChangeThreshold) and iteration < maxItr:
    ...
    for example in examplesTest:
        ...
        #feedforward over all the examples of the testing set
        for value in output:
        #for each value of the outputlist, compare if it is the same as the ground truth
            if round(value) != example[1][i]:
                correct = False
            #look for a value that does not match
            i += 1
        if correct:
            testCorrect += 1
    testAccuracy = testCorrect / (len(examplesTest) * 1.0)
