...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        #for all possible ghost positions
            emissionProb = self.getObservationProb(noisyDistance, pacmanPosition, p, self.getJailPosition())
            #find probability of observation of the current possible ghost position
            self.beliefs[p] = self.beliefs[p] * emissionProb #update agent's beliefs 
        self.beliefs.normalize() #normalize the beliefs
        #raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.allPositions: #for all possible ghost positions
            newPosDist = self.getPositionDistribution(gameState, p)
            #get distribution over new positions for the ghost using the previous position p
            for pos in newPosDist: # for possible ghost positions with their probability
                beliefs[pos] += self.beliefs[p] * newPosDist[pos] 
                #update beliefs

        beliefs.normalize() #normalize beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while end == False:
            ...
            #for all legal positions
                ...
                #if i is more than the number of particles
                    end = True #end loop
                    break
                else:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        #for all ghost positions
            emissionProb = self.getObservationProb(observation, pacmanPosition, p, self.getJailPosition())
            #find probability of observation of the current possible ghost position
            tmp[p] += emissionProb
            #update tmp by adding in emission probability
        if tmp.total() == 0:
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            newPosDist = self.getPositionDistribution(gameState, p)
            #distribution over new positions for the ghost using the previous position p
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        cartProduct = list(itertools.product(self.legalPositions, self.legalPositions))
        #list of permutations
        random.shuffle(cartProduct)
        #shuffle the list of permutations to make sure particles are spread out evenly
        end = False
        i = 0
        while end == False:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        #for all ghost positions
            ...
            #loop over all the ghosts
                emissionProb *= self.getObservationProb(observation[i], pacmanPosition, p[i], self.getJailPosition(i))
                #find the probability of an observation of the current ghost
            tmp[p] += emissionProb
            #update tmp
        if tmp.total() == 0:
        #if all particles receive zero weight
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            #loop over the ghosts
                ...
                #get the distributions of new positions for the current ghost i
